...
#define OLED_SET_MULTIPLEX_RATIO			0xA8	//Set the MUX ratio to N+1 MUX.  Next byte defines MUX from 16-64.  (Default is 0b111111 i.e. 64MUX).
	#define OLED_DEFAULT_MULTIPLEX_RATIO		0x3F	//I.e. 64MUX.
...
#define OLED_SET_DISPLAY_OFFSET				0xD3	//Set vertical shift by COM from 0 to 63 defined by next byte.  Default is 0.
	#define OLED_DEFAULT_DISPLAY_OFFSET		0x00	//I.e. 0 offset.
#define OLED_SET_COM_PINS_HARDWARE_CONFIG		0xDA	//Set COM pin config with next byte.  Default is 0b00100010 - Alternative COM pin config, disable COM left/right remap.
	#define OLED_DEFAULT_COM_PINS_HARDWARE_CONFIG	0x12	//I.e. alternative COM pin config, disable COM left/right remap.
...
{
	public:
		ssd1306();								//To initialise an ssd1306 oled driver sensor.
		void init();								//Initialise an hdc1080.
		void send_command(uint8_t command);					//Send (write) a command to the ssd1306.	
		void send_command(uint8_t command, uint8_t value);			//Send (write) a command to the ssd1306 with a corresponding command value.
